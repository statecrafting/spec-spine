---
id: "080-an-unresolved-claim-is-not-stale"
...
Every line of that report is correct: spec 079 fixed the prose. The last line
...
Exit 1 is already defined as validation failure, and spec 062 §3.3
...
Spec 069 §3.1 states, of `index check`: "The existing
blocking-diagnostic refusal stays. The exit code is unchanged: 2 when anything
drifted." That sentence is approved and adopters branch on it. Spec 079
...
(spec 037). Nothing in `specs/069-the-committed-index-is-compared-not-trusted/spec.md` is
touched.
...
verb decides from the partition spec 079 §3.2 built and not from the
composed `fresh` flag, which cannot tell the two refusals apart.

The order of spec 062 §3.3 is unchanged and already accommodates this:
...
This replaces the closing sentence of spec 069 §3.1. Under spec 037 the
...
Spec 079 §3.1 states the same rule in its own words and as its own MUST, so it
...
- Every report line stays exactly as spec 079 left it. This spec changes no
...
- `--fail-on-unresolved` is a different axis and is untouched: it refuses on the
  ...
  claiming territory it has not written yet (specs 023 and 044). Spec 044
...
spec 079's cases already build:
...
- **`report.index.actual`'s wording.** For a blocking-only corpus it reads
  ...
  compatibility shim spec 079 FR-009 deliberately held still. It stays held.
  Spec 079 records why the JSON surface did not need the correction the prose
...
- **The hooks.** Spec 093 made both session hooks read the report rather than
...
- **Every adopter's own `CLAUDE.md`.** An adopter documenting exit 2 as
  ...
  is the instrument, as it was in specs 094 and 099.
...
reverse). Spec 062 §3.3 already ranks 1 above 2 and gives the reason:
...
spec 079 removed from the prose.
...
the class of defect spec 062 exists to remove.
