---
id: "066-an-attestation-covers-the-territory-it-claims"
...
references:
  - { unit: { kind: file, path: "specs/039-per-spec-attestation/spec.md" }, role: context }
summary: >
  ...
  identically to an explicit `directory` unit. Spec 039 3.1 already states what should happen, that `units` carries
...
```
$ spec-spine attest --spec 093-the-harness-this-repository-runs
...
039's purpose is answering "was this spec's territory sound when its work was
...
**The failure is an I/O error, not a verdict.** Spec 039 3.1 is deliberate that
...
This spec changes no stated requirement of spec 039, so it declares no `amends`
edge. Spec 039 3.1 already says `units` lists owning units "each with the
content hash of what it resolved to", with no qualification by kind, and spec
016 predates 042, so directory and crate units existed in this corpus when that
...
`contentHash: null` and `resolution.ok: false`, per spec 039 3.1. This spec
...
error, exactly as spec 039 3.1 requires. Silently skipping an unreadable file
...
second is not optional and is not expressible as the first: spec 036 3.5 makes
...
`index.rs::resolve_id` against a set of ids; spec 015 2 calls the relationship a
...
machine is the one thing 3.4 and spec 039 3.5 exist to prevent.
...
**The short id at `attest --spec`.** `attest --spec 059` is refused while
`verify 070` and `compile --spec 059` accept it. That looks like a sibling of
this defect and is not one. `registry show 070` and `registry relationships 070`
refuse it too, so the invariant that specs 043 3.2 and 056 3.1 each assert in
...
Spec 039 3.3, which 042 5 then cites, settled this: a committed per-spec bundle
...
**The corpus-scoped `attest`.** Spec 021's whole-corpus verb never reads unit
...
**D-1 (2026-09-10): no `amends` edge on spec 039.** Considered and rejected. An
`amends` edge says the amending spec changes what the amended spec requires
(spec 037). Spec 039 3.1 requires a content hash for every owning unit and is
...
which its text does not say and which spec 016's presence in the corpus at the
time argues against.
...
(verified: spec 059's two file units hash to the same values before and after
this change).
...
Spec 043 3.2 forbids a line depending on a variable another line set, and the
...
target/release/spec-spine attest --spec 093-the-harness-this-repository-runs >/dev/null
# 3.1 its payload carries at least one real hash, not an error envelope.
sh -c 'test $(target/release/spec-spine attest --spec 093-the-harness-this-repository-runs --json | grep -c "\"contentHash\": \"") -ge 1'
...
target/release/spec-spine attest --spec 059-a-malformed-id-is-refused-not-a-panic >/dev/null
target/release/spec-spine attest --spec 059-a-malformed-id-is-refused-not-a-panic --json | grep -q '"schemaVersion": "0.1.0"'
```
