---
id: "073-a-mode-only-or-binary-change-is-a-change"
...
summary: >
  ...
  is invisible to `C-002`. Spec 071 met the same blindness while building
...
extends:
  ...
  - { spec: "071-a-change-is-classified-under-the-bases-rules", unit: "crates/spec-spine-cli/src/cmd_couple.rs", nature: additive }
  # 3.6: the ownership ratchet's input universe gains the paths it could not see.
  - { spec: "029-ownership-coverage", unit: "crates/spec-spine-cli/src/cmd_couple.rs", nature: additive }
...
Spec 071 needed the same path list for `delta` and could not use the parser, so
...
A `D` status MUST set `deleted: true` on the added entry, so the spec 029
...
(spec 071): `-c core.quotepath=false`, `--no-renames`, `--end-of-options`, and
...
071 established and this spec must not change. The `cargo test` line is **not**
