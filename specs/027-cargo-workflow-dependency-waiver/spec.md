---
id: "027-cargo-workflow-dependency-waiver"
...
- **GitHub Actions.** `.github/workflows/release.yml` is explicitly claimed by
  specs 006/008/021 (`kind: file`), so spec 008 lifts it above the `.github/`
...
under spec 008 means it is explicitly claimed by a spec (`.github/` is otherwise
...
exits 1. The claim-aware bypass verdict (spec 008) still decides which paths are
