---
id: "069-the-committed-index-is-compared-not-trusted"
...
amends:
  ...
  # bytes catches it. 024's text is not edited (spec 037). See 5, D-1.
  - "022-index-sharding"
establishes:
...
summary: >
  ...
  to; for the index that is not what is checked. Spec 028 3.1 recorded the
...
and the index. Spec 028 made the registry's freshness check byte-exact:
...
resolution, which is the case spec 028 had in mind; 3.4 keeps that cost bounded.
...
honestly; that is a legitimate route (spec 093) and a structural change a
...
`check` reports it until someone runs `index`. With the merge queue (spec 094)
...
**The registry side.** Spec 028 already compares bytes.
