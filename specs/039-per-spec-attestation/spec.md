...
references:
  ...
  - { unit: { kind: file, path: "specs/028-registry-freshness-check/spec.md" }, role: context }
summary: >
  Spec 021 attests the corpus: one payload, one hash, one optional Ed25519 seal
...
Spec 021 emits a `CorpusAttestation`: `inputsManifestHash`, `registryHash`, and
...
```json
{
  ...
  "specId": "033-configured-corpus-root",
...
- `units` lists **owning** units only, in the registry's canonical order, each
  ...
  excluded: spec 031 settled that a cited file is not a claimed one, and an
...
- `lifecycle` records the spec's own `status` and `implementation` as declared at
  ...
  since spec 014 accepts `n/a` on the way in and normalizes it on the way out, so
  ...
  local policy that spec 038 is already changing, while `status` and
...
payload and returned `0` irrespective of the verdicts inside it, and spec 021
never wrote that down. This spec does, for both scopes.

Stating it for the corpus-scoped verb is a change to spec 021's contract, since
023 owns `attest` and carried no such rule, so this spec declares
`amends: ["021-ledger-seal"]`. The alternative was to scope the MUST NOT to
...
happened to notice first. Per spec 037 the amendment is declared here and
`specs/021-ledger-seal/spec.md` is not edited; the inbound view is
`spec-spine registry relationships 021-ledger-seal`.
...
exactly as spec 021 established: `alg`, `keyId`, `signedAt`, `sig` over the
...
existing `.gitignore` entry for `.derived/attestation/` (spec 021's, unchanged).
...
- **It would need its own freshness gate.** The note's own constraint says every
  ...
  028 exists to record. That is a fourth committed tree and a fifth gate verb.
...
Spec 038 was designed around this and needs no attestation, which is why the
...
whoever proposes one. That is the same honesty spec 037 §4 records about its own
...
- `implementation: n-a` is present in the payload, not omitted, and a spec
  written `n/a` attests byte-identically to one written `n-a` (spec 014). The
...
2. The completion claim needs no bundle to be held to its territory (spec 038),
...
- **D-3 (2026-09-06): the territory in 2 was three files short, and the gate is
  ...
  narrower than its behavior section required (spec 036 D-1 was the first, for
...
- **D-4 (2026-09-06): each verdict mirrors its corpus-scoped counterpart, and
  ...
  error. That asymmetry is inherited rather than invented here: spec 021 sets
...
- **D-5 (2026-09-06): the seal is derived from the attestation's own filename.**
  Spec 021's CLI resolved a missing `--seal` to a fixed `attestation.sig` beside
