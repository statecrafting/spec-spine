...
summary: >
  ...
  103 added `amends_verification` and the template never gained it, a gap five
...
Specs 106, 107, 108, 109 and 110 each carry, in their own `## 4. Out of scope`,
the same sentence:
...
Spec 110 §4 adds that with the acceptance repairs done, this is "the one item of
...
| `amends_verification` | spec 103: `verify` resolves through it | absent |
...
- `planned: true` on a unit (spec 076), written by twelve specs. The template
...
The template fell behind because nothing connects it to the grammar. Spec 103
...
- **When to reach for it.** An approved spec's acceptance line has gone wrong:
  ...
  approved spec legitimately moved. Spec 040 forbids editing the amended file
...
- **That the replacement should assert that the amended file still carries the
  ...
  the amended spec instead. This is the practice specs 107 to 110 each adopted
...
merge, which is the failure mode the acceptance audit of specs 106 to 110 spent
nine PRs on.
...
- **The adopter scaffold template.** `scaffold.rs::spec_template()` emits a
  ...
  one edge of the eight on purpose. It is spec 006's territory, a different
...
- **Giving `code_aliases` and `feature_branch` meaning.** §3.2 documents what
  ...
  this spec's.
- **The green literal pins in specs 054's and 105's blocks.** Named as standing
  in spec 106 §4 and unchanged here: each is another spec's acceptance, and a
...
- **Rerunning a merged acceptance block.** §3.5 puts this spec's own check in
  ...
  Spec 105 §4 declined to move `verify` into the gate chain and recommended a
...
than by the exit code (spec 106 D-7).
...
# report that as a JSON decode error naming the wrong defect (spec 107 D-4).
...
grep -qF '111-the-template-teaches-the-whole-grammar' "${TMPDIR:-/tmp}/ss111-owner.txt"
