...
summary: >
  ...
  drift` and exits 0, which reads as a pass. Spec 090 §4 placed the defect out
  of scope and said it "is filed separately"; spec 092 §4 placed it out of scope
...
amends: ["090-a-hook-bound-to-a-tool-route-misses-the-work"]
extends:
...
Observed in this repository on 2026-09-16 while building spec 100, with
...
spec 092's rule holds for the new half as well: the parser is the authority for
...
### 3.5 What spec 090 §4 now says

This replaces the second and third sentences of spec 090 §4's staged-diff
clause. Under spec 040 the replacement text lives here, and 090 is not edited.
Where 090 reads:
...
> written, despite the sentence that stood here saying it was; spec 102 files
...
The rest of 090 §4 stands, §3.2 included. Nothing about spec 090's own
...
- **Amending spec 092 §4.** Its sentence says spec 090 "named this separately",
...
- **The harness adopting the flag.** Whether this repository's `pre-commit`
  ...
  090 §3.2's refusal to paper over the gap is unaffected either way.
- **`git stash`-based approaches.** Spec 085's fail-first probe used a partial
...
gate's answer over nonsense input is worse than no answer. Spec 063 maps every
