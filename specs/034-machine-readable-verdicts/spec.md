---
id: "034-machine-readable-verdicts"
...
extends:
  ...
  - { spec: "032-stdout-closed-reader", unit: "crates/spec-spine-cli/src/out.rs", nature: additive }
  # The envelope's schema constant and its re-export; 000 floors the types crate
  # (the same additive shape specs 011, 013 and 032 used).
...
references:
  ...
  - { unit: { kind: file, path: "specs/009-registry-query-projection-flags/spec.md" }, role: context }
...
and since spec 009 the ones with structured payloads take `--json`
...
The write goes through `out::block` (spec 032). `spec-spine couple --json | head`
MUST exit `0`, not `101`.
