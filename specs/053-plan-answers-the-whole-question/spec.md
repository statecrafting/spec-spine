---
id: "053-plan-answers-the-whole-question"
...
extends:
  ...
  - { spec: "035-registry-plan-ready-set", unit: "crates/spec-spine-cli/tests/cli.rs", nature: additive }
references:
...
Spec 035 built `registry plan` to answer the scheduling question, and it
...
Spec 035 defines what the ready and blocked sets contain and the order of both.
...
that spec's authored `depends_on` order. Spec 035 §3.2 requires the report to be
...
claims. `spec-spine verify <id>` (spec 043) is the verb that checks whether the
