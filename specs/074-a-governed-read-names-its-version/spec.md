---
id: "074-a-governed-read-names-its-version"
...
summary: >
  ...
  sorted keys, and spec 034 gave every verdict verb a versioned envelope. The
...
extends:
  ...
  # 3.3: `coverage_json` now emits the read layout, so spec 075's assertion on
  # its member spelling parses the document instead of matching compact text.
  - { spec: "029-ownership-coverage", unit: "crates/spec-spine-core/tests/coverage.rs", nature: corrective }
...
Spec 034 gave every verb that renders a verdict a `--json` envelope carrying
...
It changes **no DTO**. Spec 048 §3.3 established the rule this follows: a value
...
and the CLI cannot emit different shapes. Spec 050 §3.3 established that pairing
...
Two of the four are shapes an **approved** spec states, so under spec 037 this
spec carries an `amends` edge to each and the replacement text lives here. The
amended documents are not edited.

Replacing spec 009 §3.1's second bullet:
...
Replacing spec 053 §3.2's second paragraph:
...
emitted member, and a rename is a breaking change to spec 047's contract that
...
being wrapped in the spec 034 verdict envelope. The envelope was rejected for
...
document is written, following spec 048 §3.3, so no committed shard and no
...
shape to use. The cost is the `amends` edge to spec 053.
...
facade returns a bare array of ids (spec 010) where `index orphans --json`
returns the `{ orphaned, inFlight }` object spec 052 gave the CLI. The emitter
...
claimed by an `extends` edge on spec 057's unit. Its table also listed the
...
`index orphans --json` as a bare array, which it has not been since spec 052.
