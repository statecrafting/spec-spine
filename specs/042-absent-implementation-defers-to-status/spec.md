---
id: "042-absent-implementation-defers-to-status"
...
references:
  - { unit: { kind: file, path: "specs/035-registry-plan-ready-set/spec.md" }, role: context }
  - { unit: { kind: file, path: "specs/038-completion-held-to-claims/spec.md" }, role: context }
  - { unit: { kind: file, path: "specs/041-in-progress-is-in-flight/spec.md" }, role: context }
...
Spec 035 3.1 partitions the corpus for scheduling and says of the key:
...
Spec 038 3.1 writes the in-flight predicate down as an exhaustive table, and its
...
`complete` one is. `index.rs::in_flight` agrees with the table. Spec 041 3.1
...
`query.rs::schedulable` and `query.rs::blocker_state` (spec 035's), their tests,
and the scaffold's bootstrap spec and spec template (spec 095's). No DTO
...
The `draft` row is spec 035's original reading, kept where it was correct. A
...
**Fixing the prose slips in 041 3.5 and 044 3.3.** Spec 037: an amendment
