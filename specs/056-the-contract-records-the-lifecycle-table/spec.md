---
id: "056-the-contract-records-the-lifecycle-table"
...
extends:
  ...
  # section units spec 040 established are claimed rather than amended.
  - { spec: "040-governance-document-gaps", unit: "standards/spec/contract.md", nature: additive }
references:
...
`complete` and `n-a` count as shipped. Spec 040 quoted that section and said the
...
vocabulary spec 040 §3 describes for a governance file. The contract is on the
coupling gate's built-in bypass floor as part of `standards/`, so these are
ledger facts rather than gate-enforced ones.

Nothing in specs 035, 041, 044 or 045 is edited. This spec states in one place
...
- **An absent `implementation` is not a third value.** It defers to `status`,
  ...
  ready forever (spec 042), and which is why `n-a` exists for a record spec that
...
a declared key's value is preserved verbatim into the registry (spec 012), and
...
This is the pattern spec 040 §3.4 established when it added the amendment
...
> `amends_verification`, so `spec-spine verify 056` builds its plan from that
