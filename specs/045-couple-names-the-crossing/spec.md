---
id: "045-couple-names-the-crossing"
...
extends:
  ...
  # gate, not behavior changed. 005's spec.md is not edited (spec 037).
...
The cost of not saying so is measured. claude-observatory's spec 015 D-12
...
destroyed by formatting it into English. A consumer of the spec 034 `--json`
...
frontmatter, which is what specs 008, 030 and 050 each do. What the gate cannot
...
`VERDICT_SCHEMA_VERSION` does not move **for this change**. Spec 044 §3.6
...
Spec 034 draws this line already, and `cmd_couple.rs` records it: the envelope
...
error spec 093 found in three of the kit's hooks and which `compile --check`
...
**Per-payload schema versioning.** Spec 044 §3.6 declined to open that axis and
...
owns (spec 004 among others) and which spec 045 does not claim, so the gate
...
# 3.4: a payload addition does not move the envelope version (spec 044 3.6).
...
# read a legitimate additive bump elsewhere (spec 049 took it to 0.3.0 by
