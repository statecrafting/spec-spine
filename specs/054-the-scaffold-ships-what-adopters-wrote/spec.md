---
id: "054-the-scaffold-ships-what-adopters-wrote"
...
summary: >
  ...
  spec 036's `state_dir` has the same missing half: the live failure it fixed
  ...
  the two-bullet stub spec 040 complained about, while the real thirty-four-line
...
Spec 036's `state_dir` has the identical missing half. 039 declared a root that
...
**The constitution stub.** Spec 040 §1.4 complained about it and did not fix it.
...
spec 040 made writable, and the seam saying which principles are the adopter's
own.

Spec 040 §3.4 already updated `CONSTITUTION`, the scaffolded constitution
...
**Writing `AGENTS.md` or shipping the kit.** `init --with-kit` is spec 095.
This spec fixes the files `init` already writes.

**A `Makefile` or a CI workflow.** Spec 094.
...
**default value itself** carries the broken glob form spec 050 found in this
...
> `amends_verification`, so `spec-spine verify 054` builds its plan from that
