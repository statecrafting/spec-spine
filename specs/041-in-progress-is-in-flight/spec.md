---
id: "041-in-progress-is-in-flight"
...
references:
  ...
  - { unit: { kind: file, path: "specs/023-unresolved-unit-severity/spec.md" }, role: context }
summary: >
  Spec 023 grants an in-flight spec leniency: an unresolved owning unit is a
  ...
  file it has not written. Spec 038 3.1 wrote that cell down and 4 called it
...
Spec 023 3.1 arm 2 downgrades an unresolved **owning** unit from a blocking
...
038 made `implementation: complete` defeat that leniency, and left the predicate
...
Compare the case spec 038 *did* argue. `complete` is a claim that the work is
...
That is not true of every adopter, and spec 038 4 said so before an example was
in hand:
...
fields, exactly as spec 038 did for a different combination.
...
| draft | complete | no | unchanged (spec 038) |
...
### 3.2 Why this does not weaken spec 038
...
`pending` for this purpose (spec 035 3.1 reads it the same way for scheduling).

## 4. Out of scope

**The `approved` + `pending` combination.** Already in flight, unchanged, and
spec 038 4 explains why it stays that way.

**Verifying that code matches a spec's prose.** No gate does this. Spec 038 3.3
