...
created: "2026-09-17"
summary: >
  Spec 059's `## Verification` block asserts that `index orphans --json` has
  exactly the keys `orphaned` and `inFlight`, and spec 093 legitimately added a
...
amends_verification: ["059-read-verbs-on-a-code-free-corpus"]
references:
...
verify: 059-read-verbs-on-a-code-free-corpus: FAILED at command 4 (exit 1)
```
...
Spec 093 routed every read document through one emitter that sorts keys and
...
version literal: spec 093's calendar assertion (corrected by 103), spec 050's
version pin (corrected by 106), spec 056's version pin (corrected by 107), and
this. Spec 106 1.2 states the general form.
...
Spec 093 changed the shape of a document another approved spec's acceptance
...
chain deliberately (`AGENTS.md`); CI never runs it. Spec 106 1.3 records the
...
### 3.1 What spec 059's acceptance now is

This spec's `## Verification` block MUST replace spec 059's in full, through
`amends_verification` (spec 103 3.1), and 059's file MUST NOT be edited. The
...
spec 105 D-1: a reader asking what 059 accepts today should find one block that
...
- it is an object whose keys are **sorted**, which is spec 093 3.2's rule for
...
These are assertions about spec 059's own territory, not new territory, and they
are added for the reason spec 106 D-3 gives: a block that asserts a consequent
...
Spec 059's file MUST still carry the superseded key-set equality. That is the
spec 040 3.1 half, and it is the assertion that goes red if someone ever
...
### 3.6 No `amends` edge was owed to 059 by spec 093, and none is claimed here

This spec carries `amends: ["059-..."]` because spec 103 3.1 requires every
...
- **Editing spec 059, or spec 093.** 1.2 and the frontmatter comment. 059 keeps
...
- **Asserting the id ordering inside each group.** 059 3.1 requires each group to
  stay id-sorted, per spec 011 3.3, and neither block asserts it. On this
...
- **The other two red blocks.** Specs 060 and 071 are red for the same family of
  reason and each needs its own amendment, because `amends_verification` replaces
  a target's whole block with the amender's single one. Spec 106 4 names all
  four; 107 took 056 and this is the second.
- **The remaining green literal pins.** Spec 054's block pins `config_version` at
  `0.1.0` and spec 105's carries a `schemaVersion=='0.4.0'` inherited from 098.
...
(`verification-block-must-fail-before-the-build`, spec 106 D-7), and adding a
...
the line passes in the wrong direction. This is spec 106 D-4's hazard with the
...
gap, and it is spec 106 D-5's finding restated at a third site.
...
documents beside it carry the state instead. This block is spec 059's acceptance
...
```verify:cli
# --- spec 059's acceptance, which this block now holds (3.1) ---
# Self-contained: the commands below invoke the release binary.
cargo build --release --locked
cargo test -p spec-spine-core --test render --locked
cargo test -p spec-spine-core --test coverage --locked
# 3.1 of spec 059, as 3.3 here reads it: `orphans --json` is an object carrying
...
# spec 093 added `schemaVersion` to every governed read (3.3).
...
# 3.2 + 3.3 of spec 059: the empty-universe fixture. Built once at a fixed path,
...
# 3.2 of spec 059: without the flag it still exits 0 and reports the fact. The
...
# 3.2 of spec 059: with the flag it refuses. An assertion over an empty set is
...
# 3.3 of spec 059: and the message names which of the two empty cases this is.
...
# 3.4 of spec 059: this repository's own coverage assertion is unaffected, so
...
python3 -c "import json; d=json.load(open('${TMPDIR:-/tmp}/ss108-show.json')); assert d['amendsVerification'] == ['059-read-verbs-on-a-code-free-corpus'], d; assert d['amends'] == ['059-read-verbs-on-a-code-free-corpus'], d"
rm -f "${TMPDIR:-/tmp}/ss108-show.json"
# Spec 059's file is not edited (spec 040 3.1): its own block still carries the
# superseded key-set equality. This goes red the moment someone resolves this by
# editing 059 instead.
grep -qF 'set(o) == {"orphaned", "inFlight"}' specs/059-read-verbs-on-a-code-free-corpus/spec.md
