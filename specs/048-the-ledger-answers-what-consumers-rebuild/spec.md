---
id: "048-the-ledger-answers-what-consumers-rebuild"
...
crates/spec-spine-core/src/couple.rs
  ...
  045-couple-names-the-crossing  extends     crates/spec-spine-core/src/couple.rs
...
counts for `C-001` and counts as debt for coverage (spec 029). A **header**
owner is a `// Spec:` comment in the file itself. Collapsing these into a flat
list of spec ids would reproduce exactly the ambiguity spec 029 was written to
remove.
...
one, tangled with what is and is not a hashed input, which spec 050 is about.
...
target/release/spec-spine registry show 022-index-sharding --json | python3 -c 'import json,sys; h=json.load(sys.stdin)["contentHash"]; assert len(h)==64, h'
