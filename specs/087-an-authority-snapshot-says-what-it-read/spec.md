...
H=$(target/release/spec-spine attest --spec 083-an-attestation-covers-the-territory-it-claims --json | sed -n 's/.*"attestationHash": "\([0-9a-f]*\)".*/\1/p'); test -n "$H" && grep -q "\"specAttestationHash\": \"$H\"" .statecraft/derived/attestation/snapshot.json
