---
id: "058-the-shipped-default-hashes-what-it-names"
...
summary: >
  ...
  folds zero extra files into the content hash and is told nothing. Spec 050
  found the form in this repository's own config and fixed it there; spec 054
...
content hash is the only mechanism that was supposed to notice. Spec 021 signs
...
Two specs have already walked past this. Spec 050 found the identical form in
**this repository's** `spec-spine.toml`, fixed it here, and did not look at the
default behind it. Spec 054, writing out the exhaustive scaffolded config,
...
the form spec 054's scaffold comment already tells adopters to write, so that an
...
054's own `## Verification` block greps for, and it SHOULD name spec 058 as
...
asserts the opposite today, in spec 027's `workflow_uses_bump_auto_waives`:
...
This is not a behavior spec 058 invents. `spec-spine`'s own `spec-spine.toml`
has carried `.github/workflows/*.yml` since spec 050, so a workflow edit has
...
waive walls on exit 2. Spec 027 did not build one because it read workflow
...
other four. Spec 050 §4's separate question, whether bare `file` units should
...
**Decision, 2026-09-07: 067 is extended, not amended.** Spec 057 records, as a
...
**Decision, 2026-09-07: spec 027 is not amended.** 030 §1 says that for GitHub
...
is owned by spec 006, a crossing this spec has no other reason to make.
...
`000` package-manifest floor and claimed by seven `extends` edges. Spec 050 set
...
> `amends_verification`, so `spec-spine verify 058` builds its plan from that
...
grep -q 'Upgrading across spec 058' docs/adoption-guide.md
