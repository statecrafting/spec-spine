---
id: "038-completion-held-to-claims"
...
summary: >
  ...
  spec 023 uses it to downgrade an unresolved owning unit from a blocking
  ...
  (spec 033 did exactly that at 46474a3) and stays that way until a later
...
Spec 023 §3.1 arm 2 uses it to decide severity: an unresolved **owning** unit on
...
legitimately claim territory that does not exist yet, which is what let specs 034
...
it to `approved`. Spec 033 was filed exactly so at `46474a3` and ratified at
`f2388ed`; spec 037 sits in that state right now.
...
Neither group's tier moves. Specs 034, 038 and 039 are `draft` with
`implementation: pending` and stay in flight, which is what keeps spec 034's
claim on the not-yet-written `verdict.rs` a warning. Spec 037 is `draft` with
...
completion was claimed is a signed artifact, not a severity tier. Spec 039.
...
023 arm 2 named `draft` and `pending` and did not name `in-progress`, and nothing
...
separate purpose (§4, spec 039) and is not a precondition for holding a
