...
resolver diagnostic (spec 050's `BLOCKING_CODES`, `I-003` to `I-009`) into a
`BTreeSet`; then it compares the committed shard bytes against the recompute
(spec 086 §3.1). Both sets are flattened into one `Vec<String>` and handed to
...
`W-002`) and its exit `1`, and spec 050 §3.3's precedence (staleness outranks
unresolution for that flag) is untouched.

Spec 086 §3.1 states "the existing blocking-diagnostic refusal stays. The exit
...
Per-shard reporting is unchanged: spec 031 §3.3 makes it contractual that each
drifted shard occupies its own line carrying its drift class, and spec 086 §3.2
...
suggest `planned: true`, as a way to make the refusal go away. Spec 076 §3.3
...
report MAY name `planned: true` (spec 076 §3.2) as the declared way to hold
territory that is not written yet. That is a legitimate in-flight state under
specs 025, 041 and 044, and no contradiction exists to report.
...
- **Spec 031 §3.3** makes the stale report's *structure* contractual: "each
...
- **Spec 086 §3.1 and §3.2** require the blocking refusal to stay, the exit
  code to stay at 2, and each drifted shard to be named with its class. 3.1 and
  3.2 hold all three.
- **Spec 037's `--json` envelope** carries the same text in
