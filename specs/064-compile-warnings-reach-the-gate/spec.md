---
id: "064-compile-warnings-reach-the-gate"
...
extends:
  ...
  - { spec: "062-one-name-one-freshness-verb", unit: "crates/spec-spine-cli/src/cmd_check.rs", nature: additive }
...
summary: >
  ...
  half, the shape spec 044's `--fail-on-unresolved` already uses for the index
...
tier is a shape, not a count: the next warning-tier code, and spec 030 shows how
...
own, against a corpus whose author saw a green gate. Spec 030 wrote that
...
The edge above is therefore ordinary usage, not an inherited defect. Spec 093
...
written, and may name a `depends_on` target that is filed after it. Spec 015's
short-id resolution and spec 046's ordinal-monotonicity lint both assume that
...
`--check`, and spec 049's `--spec`. Spec 034 section 4 withholds `--json` from
...
062 moved every consumer onto the composed verb precisely so the protocol would
...
Per spec 034 the tally MUST appear in the `--json` envelope's `report`, and the
...
under spec 044, because an adopter's corpus may legitimately point forward
...
The skills' inlined floors MUST NOT be required to restate the flag. Spec 093's
...
Spec 093's subset assertion MUST continue to hold. Spec 094's
...
spec 062 deliberately moved every consumer off the primitives, so a flag on
...
disambiguates, which is the same answer spec 062 reached when `1` became
...
by design under spec 044, because an adopter's corpus may legitimately be
...
exit `3` under spec 093's usage-error mapping, and the five greps. Each grep
...
093's subset assertion in `kit_skills.rs`, which is a test line and therefore
