...
created: "2026-09-17"
summary: >
  Spec 050's `## Verification` block has three assertions that no longer hold,
  and none of the three describes a defect. It compares the whole of `index
  check`'s stdout against one line, which spec 057 legitimately added a second
  line to; it takes `len()` of `index diagnostics --json` as a bare array, which
  spec 093 legitimately wrapped in an `items` envelope; and it pins
  `VERDICT_SCHEMA_VERSION` at the literal `0.2.0`, which spec 088 legitimately
...
verify: 050-index-diagnostics-reach-a-gate: FAILED at command 5 (exit 1)
```
...
trips over is spec 057's unwitnessed-claims line, which is a second line about a
...
Command 8 is the one worth naming as a class. Spec 050 3.6 rules:
...
else moves it, which 3.6 expressly permits. Spec 088 moved it for an
...
it is an assertion of the calendar, and spec 103 3.5 corrected the same shape in
spec 093's block.
...
Specs 057, 088 and 093 each changed an output that 050's acceptance asserts on.
None declared `amends_verification`, and none could have: spec 103 built that key
and shipped it in v0.20.0, after all three had merged. The gate chain does not run
`verify`, deliberately (spec 049), so each block went red at merge time with
nothing to report it. Spec 105 gave one such block its replacement; spec 103 4
...
### 3.1 What spec 050's acceptance now is

This spec's `## Verification` block MUST replace spec 050's in full, through
`amends_verification` (spec 103 3.1), and 050's file MUST NOT be edited. The
...
Replacing the block in full rather than the three lines follows spec 103 3.5: a
...
057's line is a second line about unwitnessed claims.
...
member that spec 050 added, with its three keys.
...
Spec 050's file MUST still carry all three superseded assertion forms. That is
the spec 040 3.1 half, and those are the lines that go red if someone ever
...
spec 103 in v0.20.0; this spec is a corpus change that uses it.

## 4. Out of scope

- **Editing specs 050, 057, 088 or 093.** 050 keeps the block it was ratified
  with, which is the record spec 040 3.2 protects. The other three state rules
  that are true as written; what their changes invalidated was an acceptance,
  not a rule.
- **The other four red blocks.** Specs 056, 059, 060 and 071 are red for the
...
- **Recurring sweep coverage.** Nothing reruns a merged block, which is how
  three crossings went silent. Spec 105 4 declined to put `verify` in the gate
...
D-1 (2026-09-17, why a replacement rather than three corrections). Spec 103 3.5
settled that a block is the unit, and spec 105 D-1 carried it: a reader who finds
...
red to the next legitimate bump and this spec gets filed again, which spec 105
...
govern. One line closes that, and it is an assertion about spec 050's own
...
(`verification-block-must-fail-before-the-build`, spec 103 1.1, spec 105 D-5,
...
state instead. This block is spec 050's acceptance (3.1) as well as this spec's
...
```verify:cli
# --- spec 050's acceptance, which this block now holds (3.1) ---
...
# 3.1 (amended by 3.2): a clean corpus keeps the bare verdict line. Spec 057's
...
# 3.4: the read verb answers, and answers as JSON. Spec 093 wrapped the payload
...
# Spec 050's file is not edited (spec 040 3.1): its own block still carries all
# three superseded assertion forms. These go red the moment someone resolves
# this by editing 050 instead.
grep -qF 'index check)" = "index is fresh"' specs/050-index-diagnostics-reach-a-gate/spec.md
grep -qF 'print(len(json.load(sys.stdin)))' specs/050-index-diagnostics-reach-a-gate/spec.md
grep -qF '= "0.2.0"' specs/050-index-diagnostics-reach-a-gate/spec.md
