...
summary: >
  ...
  generalizes spec 011 (index hash slices) from scoped staleness CHECKING to
  per-unit STORAGE, and it removes the textual-conflict cause spec 094's merge
...
depends_on:
  ...
  - "011-index-hash-slices"       # generalized: a slice was a hash field, a shard is its own file
establishes:
...
extends:
  ...
  # Schema-version constants + embedded shard schemas (spec 011 owns version/schema).
  - { spec: "011-index-hash-slices", unit: "crates/spec-spine-types/src/version.rs" }
  - { spec: "011-index-hash-slices", unit: "crates/spec-spine-types/src/schema.rs" }
  - { spec: "011-index-hash-slices", unit: "crates/spec-spine-types/tests/dtos.rs" }
  - { spec: "011-index-hash-slices", unit: "crates/spec-spine-core/tests/conformance.rs" }
  ...
  # The render/orphans projection tests (spec 010).
  - { spec: "010-index-render-orphans", unit: "crates/spec-spine-core/tests/render.rs" }
  # The adopter-loop test (spec 095) asserts the package shard, not index.json.
  # The merge driver (spec 094): narrowed to the rare same-shard conflict; its
  # registration moves to the shard globs.
  - { spec: "094-one-gate-and-the-boundaries-it-holds", unit: ".githooks/merge-derived-index.sh" }
  - { spec: "094-one-gate-and-the-boundaries-it-holds", unit: ".githooks/enable-merge-driver.sh" }
references:
...
serialization point. spec 094 (the merge driver) auto-resolves the conflict on a
...
- **Per-slice sidecar.** The spec 011 `[index.slices]` hashes move from a
...
- **FR-003 (per-shard + set staleness).** `spec-spine index check` MUST verify
  ...
  *set* to the current authority set (a spec or package added/removed is a
  ...
  global-hash comparison. The spec 011 `--slice` form is retained against the
...
- **FR-005 (conflict-free by construction).** Two changes to disjoint units, when
  ...
  file, WITHOUT relying on a custom merge driver. The spec 094 driver remains
