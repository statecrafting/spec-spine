...
summary: >
  ...
  specs 106 through 110 each exist to repair one such block, and each was found
  by hand. Spec 105 §4 named the remedy, "a periodic sweep a maintainer runs,
...
extends:
  ...
  - { spec: "007-distribution", unit: "docs/releasing.md", nature: additive }
references:
...
Spec 105 §1.3 states it exactly:
...
|---|---|---|
| 093 | spec 093's block asserted a corpus-dependent value | 103 |
| 098 | spec 101 amended away the exit code five lines required | 105 |
...
looking. Spec 103 §4 called the audit "its own work, with its own findings";
spec 105 §4 called the remedy a periodic sweep and declined to name its home.
...
`047-harness-rules-name-the-legitimate-edits`: a contiguous run with no gaps
and no member above it. That is not a coincidence and not neglect. Spec 049
built the `verify` verb and spec 048, filed immediately before it, is the first
...
disappointing one: specs 105 through 111 repaired the seven blocks of §1.1
...
- `docs/releasing.md` (`extends` on spec 007): one pre-flight item naming when
...
`amends_verification` and follows the chain to its end (spec 103 §3.2), so a
...
which executes nothing (spec 049 §3.8).
...
`not-declared` MUST NOT count as success. This is the policy spec 049 §4 said
...
obliged to write a block for spec 012 this quarter, and the moment somebody
...
declares, which is strictly more code, so the trust boundary spec 049 §3.6 and
spec 105 §4 draw MUST hold and MUST be enforced rather than documented.
...
the crossing that staled every block specs 105 through 110 repaired.

## 4. Out of scope

- **Repairing anything the first run finds.** Spec 105 fixed one block and
  specs 106-110 fixed one each, and each was a spec's worth of work because
...
- **Shipping the sweep to adopters in `kit/`.** The kit's skills are pinned
  byte-identical to `.claude/skills/` (specs 048, 081) and `kit_embedded.rs` is
...
- **Changing `verify`, its grammar, its exit codes or its report.** §3.2. The
  sweep is a caller. Spec 049's verb is unchanged and this spec amends nobody.
...
- **D-1 (2026-09-17, the surface is a script, not `verify --all`).** Spec 049
...
- **D-5 (2026-09-17, the trust boundary is enforced, not documented).** Spec
  105 §4 states it as a reason; a sentence in a runbook does not stop a
