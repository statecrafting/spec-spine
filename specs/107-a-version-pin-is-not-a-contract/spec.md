...
created: "2026-09-17"
summary: >
  Spec 056's `## Verification` block pins the verdict envelope at the literal
  `0.3.0`, which spec 088 legitimately moved to `0.4.0`, so
  `spec-spine verify 056` has been red since 088 merged. The pin was never what
  056 required: 056 3.4 states that `--spec` emits the spec 037 envelope with a
...
verify: 056-compile-one-spec: FAILED at command 5 (exit 1)
```

Command 5 is

```
compile --spec 024 --json | python3 -c '... assert v["schemaVersion"]=="0.3.0"'
```

and the envelope answers `0.4.0`. Spec 088 moved `VERDICT_SCHEMA_VERSION` from
...
| 8 | `compile --spec 024 --check` exit 3 | green |
...
accepts `--json` and emits the spec 037 verdict envelope, and that the envelope
...
- It is **too strong** for the version. 056 3.4 states what that one bump was,
  ...
  permitted move. Spec 105 D-2 named the test this fails: an assertion that must
...
consecutive spec to correct one: spec 093's calendar assertion (corrected by
103), spec 050's `VERDICT_SCHEMA_VERSION` pin at `0.2.0` (corrected by 106), and
...
Spec 088 changed a constant that another approved spec's acceptance pinned, and
...
Spec 106 1.3 records the identical mechanism for 050 and names this spec's
...
### 3.1 What spec 056's acceptance now is

This spec's `## Verification` block MUST replace spec 056's in full, through
`amends_verification` (spec 103 3.1), and 056's file MUST NOT be edited. The
...
spec 105 D-1: a reader asking what 056 accepts today should find one block that
...
The scratch directory is named for **spec 056**, whose acceptance this block is,
not for this spec, which merely holds it. Spec 105 carries 098's block under
...
(contrast spec 106 D-8, which had to pick one).
...
only as an exit code: `compile --spec 024` exits 0. An exit code cannot say
...
Spec 056's file MUST still carry the superseded literal pin. That is the spec
040 3.1 half, and it is the assertion that goes red if someone ever resolves
...
- **Editing spec 056, or spec 088.** 1.2 and the frontmatter comment. 056 keeps
...
- **The other three red blocks.** Specs 059, 060 and 071 are red for the same
  ...
  single one. Spec 106 4 names all four; this spec is the first.
- **The remaining green literal pins.** Spec 054's block pins `config_version`
  at `0.1.0` and spec 105's carries a `schemaVersion=='0.4.0'` inherited from
...
- **Putting `verify` in the gate chain.** Spec 105 4 declined it and recommended
...
D-1 (2026-09-17, why the counter-verb needed no anchor argument). Spec 106 D-8
...
to defend. The `--spec 024` lines inherited from 056's block do depend on 024
...
Adding to a block being repaired is scope this spec had to justify, as spec 106
...
assertions about spec 056's own territory, not new territory.
...
structural weight is carried by the two verb tokens, which spec 037 makes part
...
independence. Spec 106 D-8 records the same distinction for the same reason, and
...
gap, and it is spec 106 D-5's finding restated at a second site: what was red at
...
056's acceptance from the moment this spec merges (3.1), and the state it
carries between lines is that block's state. Spec 105 carries 098's block under
...
state instead. This block is spec 056's acceptance (3.1) as well as this spec's
...
```verify:cli
# --- spec 056's acceptance, which this block now holds (3.1) ---
...
target/release/spec-spine compile --spec 024 --json > "${TMPDIR:-/tmp}/ss056/spec.json"
...
python3 -c "import json; r=json.load(open('${TMPDIR:-/tmp}/ss056/spec.json'))['report']; assert r['specId']=='024-index-sharding', r; assert r['specPath']=='specs/024-index-sharding/spec.md', r; assert r['violations']==[], r"
...
# 3.5 of spec 056: it wrote nothing. The committed shards are exactly as they were.
...
# editing 056 instead.
grep -qF 'v["schemaVersion"]=="0.3.0"' specs/056-compile-one-spec/spec.md
