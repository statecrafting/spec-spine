---
id: "090-the-verdict-is-the-only-thing-on-stdout"
...
summary: >
  Spec 034 3.1 requires that under `--json` a verb write exactly one JSON object
  ...
  argument spec 043 1.2 used to justify building it, so the one verb that had to
  ...
  parent's stderr as they arrive, and the command and exit transcript spec 043
...
Spec 034 3.1 is unambiguous: "Under `--json` a verb MUST write exactly one JSON
object to stdout." Spec 043 gave `verify` that envelope, and the CLI runs each
...
`ok` agrees with the exit code, the report carries the facts spec 043 3.4 lists,
...
never exercised it, which is the shape spec 084 and spec 088 both name: an
assertion that cannot fail against the input it is given asserts nothing about
the input it will meet.

### 1.3 Spec 043 3.5 asks for a transcript and gets none here

Spec 043 3.5: the CLI "MUST echo each command and its exit code, so that a CI
...
The transcript is a diagnostic, and spec 032 3.3 already decided where the CLI's
diagnostics go.

## 2. Territory

`crates/spec-spine-cli/src/cmd_verify.rs`, established by spec 043, extended
...
No engine module changes: the seam spec 043 3.1 draws is untouched, and
...
report path stdout holds one envelope; on an error path (spec 034 3.3, for
instance the `R-001` re-entry refusal of spec 043 3.7, or an `Error::Io` from a
...
The transcript spec 043 3.5 requires MUST be emitted in both modes. Its channel
...
where spec 032 3.3 puts every CLI diagnostic.

This clarifies spec 043 3.5 rather than changing it. That clause requires the
echo and names no channel; read as a requirement about stdout it is
unsatisfiable under `--json` alongside spec 034 3.1, and the implementation
...
withdrawn and spec 043's file is not edited.
...
- **Execution.** Commands run through `sh -c`, from the repository root, in plan
  ...
  (spec 043 3.5).
- **The report.** Every field spec 043 3.4 lists, `ran` and `total` included,
  and a failing command's own exit code in `failure.exitCode` and never in the
  process's status (spec 043 3.3).
...
- **Recursion protection.** `SPEC_SPINE_VERIFY_STACK` is set on every child and
  re-entry is refused with `R-001` (spec 043 3.7). A refusal under `--json` is
  an error envelope, and 3.1 applies to it.
- **`--plan --json` executes nothing.** It emits the `VerifyPlan` in the
  envelope (spec 043 3.8) and spawns no process, so 3.1 holds there trivially
...
6. A consumer that reads the opening transcript line and then **closes** the
   ...
   (D-6).
7. An error path under `--json`: the `R-001` refusal of spec 043 3.7 puts one
...
**Spec 082 3.4's "acceptance amended by" notice.** It is printed only without
`--json` today, and it is left exactly there. It is not the transcript spec 043
...
stderr under `--json` would be a change to spec 082's requirement rather than a
clarification of spec 043's. If it is wanted there it is additive and separate.

**`verify`'s place outside the gate chain.** Unchanged, per spec 043 3.6.
...
- **D-1 (2026-09-19): the transcript's channel is per mode, not global.** 3.3
  ...
  mode's stdout transcript is what `verify` has printed since spec 043 shipped,
...
- **D-3 (2026-09-19): a failed write of the forwarded bytes is not a failure of
  ...
  This follows spec 032 3.3: a process whose stderr has gone has no channel left
...
- **D-4 (2026-09-19): D-3 excuses delivering the bytes, never draining the
  ...
  panic on a failed write, for the reason spec 032 §3.2 gives about stdout.
...
# The seam spec 043 3.1 draws is still drawn: the engine spawns nothing.
