---
id: "044-index-diagnostics-reach-a-gate"
...
summary: >
  Spec 023 downgrades an unresolved unit to a counted `W-001` (owning unit on an
...
stops. Spec 023 excluded `W-001` and `W-002` from that list on purpose, and
...
- A spec that goes `complete` while still claiming an unresolved unit turns
  those warnings into hard errors all at once (spec 038), so the debt is
...
Opt-in, because specs 023 and 044 exist precisely to let a corpus that ratifies
...
034 exists so a consumer branches on the exit code or the `--json` envelope,
...
`exitCode`, and the presence of `report` or `error`. Spec 034 scopes it that
way, and its MINOR rule names `error.kind` and (since spec 043) `verb` tokens,
...
> `084-an-acceptance-outlives-the-output-it-was-written-against` declares this
> spec in `amends_verification`, so `spec-spine verify 044` builds its plan
> from that spec's block and names the substitution in `acceptanceFrom` (spec
> 082 3.2 and 3.4).
>
