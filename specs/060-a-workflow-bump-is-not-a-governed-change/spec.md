---
id: "060-a-workflow-bump-is-not-a-governed-change"
...
extends:
  ...
  # 3.3 the migration note, beside spec 058's in the same document. Declared in
...
summary: >
  ...
  re-index commit. Spec 027 built the coupling half of this for workflows and
  read the freshness half as already fine; it was not, and spec 058 made that
...
Spec 027 built exactly this mechanism for the coupling half: a workflow whose
...
files. Spec 058 fixed the glob, made the staleness real for every adopter, and
...
**This spec `amends` 069.** Section 3.6 of spec 058 requires the corrected test
...
behavior. Per spec 037 both edges are declared here only; neither amended
...
not change which files are hashed, which is spec 058's question and is settled.

### 3.2 The projection and the waiver state one rule

Spec 027 already decides what a dependency-only workflow change is, in
...
The migration note MUST state the consequence for spec 021: an attestation
...
**It MUST NOT be described as a failure that resembles tampering.** Spec 021
...
**The pip, docker and go ecosystems.** Spec 027 section 4 deferred these
...
**Which files are hashed.** Spec 058 settled the default glob and this spec
does not revisit it. A repository that does not hash its workflows is
unaffected by this change in either direction.

**Whether a bare `file` unit should contribute its bytes.** Spec 050 section 4
...
**2026-09-08: `risk: high`, above the `medium` its neighbours carry.** Spec 058
...
invisible to the ledger. Spec 027's waiver already draws the line in the same
...
implementation branch outstanding, spec 093 having merged before this branch
