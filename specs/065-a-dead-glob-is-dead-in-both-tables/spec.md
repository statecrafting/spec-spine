---
id: "065-a-dead-glob-is-dead-in-both-tables"
...
extends:
  ...
  # talking about. Claimed the way spec 061 claimed the same file when it
...
`L-010` (spec 061 §3.1) refuses an `[index] extra_hashed_inputs` pattern ending
...
`[index.slices]` (spec 011) holds pattern lists too. `IndexConfig::slices` is
...
Spec 058 fixed the shipped default to the `**/*` form and, by its own §4,
...
- `crates/spec-spine-core/src/lint.rs`, through `003-conformance-lint`, for the
  rule and its message. Spec 061 carries the same unit through the same spec.
- `crates/spec-spine-core/tests/lint.rs`, through
  `046-depends-on-ordinal-monotonicity`, for the acceptance.
...
hash". That is true of `extra_hashed_inputs` and **false of a slice**: spec 011
...
**Correcting spec 011's own example.** `specs/011-index-hash-slices/spec.md`
...
**Folding slices into `contentHash`.** Their independence is spec 011's design,
...
`003-conformance-lint`, not through `061-shipped-is-not-the-same-as-working`,
even though 074 is where `L-010` was established, and the test file is claimed
through `046-depends-on-ordinal-monotonicity`. Each unit is routed through the
...
not consistent about it: specs 050 and 058 carry `tests/lint.rs` through 003,
...
Each line is one command. Spec 043 §3.2 makes a fence's body line a command, so
...
preceding command's status in the same shell. Spec 052's block, approved and
