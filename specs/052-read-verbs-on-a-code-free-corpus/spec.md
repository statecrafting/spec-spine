---
id: "052-read-verbs-on-a-code-free-corpus"
...
extends:
  - { spec: "010-index-render-orphans", unit: "crates/spec-spine-core/src/render.rs", nature: additive }
  ...
  - { spec: "010-index-render-orphans", unit: "crates/spec-spine-core/tests/render.rs", nature: additive }
  ...
  # empty-case silence.
  - { spec: "010-index-render-orphans", unit: "crates/spec-spine-cli/tests/cli.rs", nature: additive }
references:
...
and rahi (22 specs, one wave built). Specs 038 and 044 were written for exactly
...
Neither spec 010 nor spec 029 is edited. 011 requires `orphans` to report the
...
The predicate is the one `index.rs` already computes and spec 041 defined:
...
Ordering inside each group stays id-sorted, as spec 010 §3.3 requires.
...
unhashed. That is a real gap, it is the same gap spec 050 measures from the hash
...
definition spec 010 gave and this spec keeps it. Only the presentation
partitions.
...
> `086-an-exact-key-set-refuses-what-the-rule-allows` declares this spec in
> `amends_verification`, so `spec-spine verify 052` builds its plan from that
...
Each line below is one command. Spec 043 §3.2 is explicit that a fence's body
