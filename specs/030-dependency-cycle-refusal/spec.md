---
id: "030-dependency-cycle-refusal"
...
- **Short ids resolve first.** The detector reads the records, not the parsed
  frontmatter, so a cycle written with spec 015's short form (`depends_on:
...
that shard, which is precisely what spec 022's sharding exists to prevent. It
