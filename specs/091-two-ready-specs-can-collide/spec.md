...
extends:
  ...
  - { spec: "060-plan-answers-the-whole-question", unit: "crates/spec-spine-cli/src/cmd_registry.rs", nature: additive }
  # 3.7: the acceptance.
...
`plan` partitions the corpus with `depends_on` (spec 038). A spec is ready when
...
`constrains`. `references` is non-owning (spec 034) and MUST NOT contribute.

A unit marked `planned` (spec 076) counts. A spec that has declared territory it
...
and each pair's units sorted by identity string. Spec 038 3.2 requires the whole
...
grep -qF 'references' specs/091-two-ready-specs-can-collide/spec.md
