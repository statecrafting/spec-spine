...
created: "2026-09-17"
summary: >
  Spec 060's `## Verification` block compares the whole of
  `registry plan --next --json` against one spec object, and spec 093 moved that
  ...
  acceptance, because spec 040 forbids editing the amended file and the block
...
amends_verification: ["060-plan-answers-the-whole-question"]
references:
...
right to. Spec 093 then **changed the contract**, deliberately and with its
...
specs 056, 059 and 071: 093 amended the rule and could not reach the acceptance,
because spec 040 forbids editing the amended file and the block lives inside it.
That is spec 105 1.2's finding, which was about spec 101 and spec 098, restated
at a second site. Spec 103 built `amends_verification` after 093 shipped, so the
...
Spec 093 changed a document shape another approved spec's acceptance pinned,
...
deliberately (`AGENTS.md`); CI never runs it. Spec 106 1.3 records the identical
...
### 3.1 What spec 060's acceptance now is

This spec's `## Verification` block MUST replace spec 060's in full, through
`amends_verification` (spec 103 3.1), and 060's file MUST NOT be edited. The
...
spec 105 D-1: a reader asking what 060 accepts today should find one block that
...
- the document's keys are **sorted**, which is spec 093 3.2's rule for every
  governed read;
...
Spec 060's file MUST still carry the superseded whole-document equality. That is
the spec 040 3.1 half, and it is the assertion that goes red if someone ever
...
### 3.6 The empty ready set stays where spec 093 put it
...
- **Editing spec 060, or spec 093.** 1.2 and the frontmatter comment. 060 keeps
...
- **The last red block.** Spec 071 is red for the same family of reason and needs
  its own amendment, because `amends_verification` replaces a target's whole
  block with the amender's single one. Spec 106 4 names all four; 107 took 056,
...
the same line and asserts both members. Spec 103's block, which is 093's
...
D-2 (2026-09-17, why this spec does not read as a loosening). Specs 107 and 108
...
leaving the projection rule unasserted and noting the gap in 4, is what spec 108
...
new assertion needs two documents at once, which is where spec 106 D-4 records
the hazard turning silent: two failures can compare equal. The `registry show`
line is included for the reason spec 107 D-4 gives, which review established on
...
gap, and it is spec 106 D-5's finding restated at a fourth site.
...
captured beside it carry the state instead. This block is spec 060's acceptance
...
**Spec 060's decision of 2026-09-08 is carried across.** The shape assertions run
...
```verify:cli
# --- spec 060's acceptance, which this block now holds (3.1) ---
...
# that document's `next: null` path is guarded by spec 093 3.8 in
...
python3 -c "import json; d=json.load(open('${TMPDIR:-/tmp}/ss109-show.json')); assert d['amendsVerification'] == ['060-plan-answers-the-whole-question'], d; assert d['amends'] == ['060-plan-answers-the-whole-question'], d"
rm -f "${TMPDIR:-/tmp}/ss109-show.json"
# Spec 060's file is not edited (spec 040 3.1): its own block still carries the
# superseded whole-document equality. This goes red the moment someone resolves
# this by editing 060 instead.
grep -qF 'assert json.load(sys.stdin)=={"id":"001-alpha","title":"First thing"}' specs/060-plan-answers-the-whole-question/spec.md
