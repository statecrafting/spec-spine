---
id: "057-the-docs-name-what-adopters-derived"
...
extends:
  ...
  # change to a claimed document stales the ledger (spec 050).
references:
...
summary: >
  ...
  ownership recursively. And spec 034 changed `attest`'s stdout, which
...
`pending` for months. Specs 038 and 044 were built for that mode. No document
...
- *A trailing-slash directory unit in `establishes` satisfies ownership
  ...
  retire coverage debt and it is documented only in spec 016's own text.

**Spec 034 has no migration note.** It made every verdict verb emit a canonical
...
- **The lifecycle table**, by reference to the contract section spec 056 adds
...
- **The guarded composite gate**: pointing at `kit/Makefile` and `kit/govern.yml`
  (spec 094) rather than restating them, and naming the `hashFiles` finding once
...
`docs/schema-versioning.md` MUST gain a migration note for spec 034 saying, in
...
**Writing the Makefile or the workflow.** Spec 094. This page points at them.

**A second lifecycle table.** §3.1. The contract holds it (spec 056).
...
bypass (spec 008). The edge would have made every future README edit a `C-001`
