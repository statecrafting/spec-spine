---
id: "076-a-stray-shard-is-orphaned-at-the-verbs"
title: "A stray shard is orphaned at the verbs"
status: approved
kind: "tooling"
created: "2026-09-14"
summary: >
  Spec 069 §3.1 says `orphaned` covers "a stray file in either shard
...
`check_report` (the composed verb spec 062 named) does this, in order:

1. compile, compare the committed registry, build the registry half;
2. `check_index_freshness`, which after spec 069 compares committed shard bytes
...
expect it, and after spec 069 a file the recompute does expect is compared byte
...
- `VERDICT_SCHEMA_VERSION` does not move, following spec 044 §3.6.

The omission is the same treatment spec 063 §3.6 gives `plannedTerritory` and
...
CLI arm MUST all reach this behavior through the same function. Spec 050 §3.3
...
- `compile --check` and the registry half are untouched: the registry's
  ...
  freshness object by design (spec 044's note on this payload).
...
spec 028 §3.3's contract and a consumer reads the shard names from it. This spec
...
file that will not deserialize is *stale* by spec 069's byte comparison. The
...
signature (spec 044's tests call it) and becomes best-effort, erroring only on
...
refused there: spec 069 D-5's refusal still happens in the freshness
