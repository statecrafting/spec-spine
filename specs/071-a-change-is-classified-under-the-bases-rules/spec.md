---
id: "071-a-change-is-classified-under-the-bases-rules"
...
changes, and `extends` is how a spec claims a unit it must touch (spec 093).
...
I/O, git or parse trouble. It is a record, not a gate (spec 039 3.1's rule for
...
committed index (read through the loaders and compared under spec 069 before
...
with `owners_for_path` (spec 048) against the base index and against the head
index, whole-file.
...
| `verification` | a `spec.md` whose `verify:cli` plan (spec 043's parser) differs between base and head, including a spec added or removed |
...
`VERDICT_SCHEMA_VERSION` to its next MINOR (spec 034's own rule). The facade
...
candidate's to write, and spec 069 exists because a committed body can say
...
else (spec 093), and a report computed under an index that does not match its
...
013) and the full-scope `supersedes` spelling (spec 018) compare as the edges
...
file (spec 058), and a dead glob is dead everywhere (spec 065). A path is
