---
id: "031-references-non-owning-paths"
...
summary: >
  ...
  `specs/094-one-gate-and-the-boundaries-it-holds/spec.md` was refused as `C-001` with
  `022-index-sharding`, which only cites it, named as the sole owner. This spec
...
| coverage (spec 029) | a claimed source file |
...
On this corpus the defect is reachable today. `022-index-sharding` references
`specs/094-one-gate-and-the-boundaries-it-holds/spec.md` for context. Changing spec
094's own `spec.md` was therefore refused:

```
C-001 'specs/094-one-gate-and-the-boundaries-it-holds/spec.md' changed without an
authoring edit to any owning spec (022-index-sharding)
```
