---
id: "063-planned-territory-is-declared-not-inferred"
...
depends_on:
  ...
  - "061-shipped-is-not-the-same-as-working"   # allocates L-010; this spec takes the next two
amends:
...
extends:
  ...
  # 3.1 spec 013's `paths:` sugar expands to file units, and those units now
  ...
  - spec: "013-edge-paths-grammar-sugar"
    nature: additive
    paths:
...
summary: >
  ...
  from a unit resolving on disk, so a corpus that opts into spec 044's
...
unresolved unit, and spec 023 grades it by lifecycle: a warning while the
owning spec is in flight, an error once it is not. Spec 044 then offers
...
cannot declare territory before writing it at all.** Filing spec 062 with
...
change. Specs 093 to 075 are `extends`-only, and that is not a property of
...
an additive minor with no schema-file edit." Spec 016, which added those three
...
marked planned still classifies exactly as spec 023 requires: `W-002` from a
...
keeps it from becoming a state nobody owns the exit from. Spec 038 established
...
**MINOR, `1.1.0` to `1.2.0`**, following the precedent spec 026 set when the
...
**Retrofitting the corpus.** Specs 093 to 075 deferred their claims to their
...
payload is permissive, and spec 016 extended the vocabulary with no amendment.
...
`L-010` is allocated by spec 061, which is why this spec takes the next two and
depends on it.
...
nothing, which is the exact defect spec 061 was filed about. Moved, and recorded
