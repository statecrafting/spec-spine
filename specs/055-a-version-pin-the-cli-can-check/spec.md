---
id: "055-a-version-pin-the-cli-can-check"
...
summary: >
  ...
  see spec 041, which was written for rahi's exact state, because it is pinned
...
- **rahi cannot see spec 041.** 044 was written for rahi's exact `approved` plus
...
- **A pin is not just about features.** The built-in bypass floor is a constant
  ...
  and spec 047 makes the floor readable but cannot make an old binary's floor
  match a new one's.
...
file already governs the repository, `spec-spine config show` (spec 047) reports
...
047 §3.4 mirrors `Config`'s tables one for one in `EffectiveConfig`, and its
...
accident. Spec 054 §3.2 sets the rule this follows: a key that only makes sense
