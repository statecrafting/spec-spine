...
extends:
  ...
  - { spec: "045-couple-names-the-crossing", unit: "crates/spec-spine-cli/tests/couple.rs", nature: additive }
...
mistake spec 028 was written about and spec 093 found in three of the kit's
...
mode that validates without writing, which spec 028 already established is a
...
Spec 045's `## Verification` carried the same literal, and was rewritten the
same way: it now asserts that `couple`'s envelope version equals `lint`'s,
which is the claim spec 044 §3.6 actually settled (the constant is
...
`<id>` accepts the short form (spec 015): `056` resolves to `049-compile-one-spec`
...
`--spec` MUST accept `--json` and emit the spec 034 verdict envelope, with a
...
Spec 043 set the precedent when `verify` was added, and the reasoning has not
...
028 exists to hold, in order to work around the absence of the verb this spec
adds.
...
target/release/spec-spine compile --spec 094
...
target/release/spec-spine compile --spec 094 --json | python3 -c 'import json,sys; v=json.load(sys.stdin); assert v["verb"]=="compile.spec", v; assert v["schemaVersion"]=="0.3.0", v'
...
target/release/spec-spine compile --spec 094 --check ; test $? -eq 3
```
