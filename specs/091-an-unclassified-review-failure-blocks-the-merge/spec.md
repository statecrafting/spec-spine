---
id: "091-an-unclassified-review-failure-blocks-the-merge"
...
references:
  ...
  # (spec 031, D-1): the file's own header records the decision not to put a
...
A `references` edge is the corpus's only non-owning edge (spec 031): the
...
- **Any judgement of review quality.** 3.4 establishes output presence only.
  ...
  not touch, and spec 093's reviewer-reading discipline is where that lives.
...
in spec 094's build: a successful filter is not a successful command.
