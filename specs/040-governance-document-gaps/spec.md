---
id: "040-governance-document-gaps"
...
establishes:
  # The answer spec 037 4 left open, stated where a reader checking the edge
...
references:
  - { unit: { kind: file, path: "specs/037-amendment-authoring/spec.md" }, role: context }
  - { unit: { kind: file, path: "specs/038-completion-held-to-claims/spec.md" }, role: context }
summary: >
  ...
  where it is equally unwritable. Spec 037 4 named this as an open question
...
["030-dependency-cycle-refusal"]`. The constitution is `standards/spec/constitution.md`.
...
Spec 037 4 saw this and set it aside explicitly:
...
|---|---|---|
| `depends_on` | a cycle is a compile-time refusal | spec 030 |
| `depends_on` | the topological order of the ready set | spec 035 |
| `implementation` | which specs a scheduler may hand out | spec 035 3.1 |
| `implementation` | whether an unresolved owning unit is `W-001` or a hard error | spec 038 3.1 |
...
- `standards/spec/contract.md`: one new `## Amending the constitution` section,
  ...
  037 used for the section beside it.
- `crates/spec-spine-core/src/scaffold.rs` and its integration test: the
  ...
  constitution gains the seam 1.4 describes. Additive to spec 095's territory.

Spec 000 is **not** edited. Its 3 classification is amended by the edge in this
spec's frontmatter and by nothing else, which is the rule spec 037 3.1 states.
...
#### 3.1.2 The constitution is edited in place, and why that is not spec 037's rule reversed

Spec 037 3.1 forbids editing an amended spec to record that it was amended.
...
`scaffold_init` (spec 095) is additively updated:
...
- *The ordinal as build order.* hqgit's bootstrap 2 requires every `depends_on`
  ...
  because spec 030 refuses cycles only. That is a genuine tooling gap, but the
...
- *`## Verification` blocks.* hqgit's specs carry `verify:cli` fenced blocks and a
  ...
  document correction. Spec 039 already moves in that direction. Filing it
