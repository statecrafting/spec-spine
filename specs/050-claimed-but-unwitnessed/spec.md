---
id: "050-claimed-but-unwitnessed"
...
extends:
  ...
  - { spec: "044-index-diagnostics-reach-a-gate", unit: "crates/spec-spine-cli/src/cmd_index.rs", nature: additive }
  # The allowlist knob (3.5), beside spec 046's in the same table.
  - { spec: "046-depends-on-ordinal-monotonicity", unit: "crates/spec-spine-types/src/config.rs", nature: additive }
...
summary: >
  ...
  021 signs an attestation over a ledger that never hashed any of them. This
...
This is measurable here, not inferred. `AGENTS.md` is claimed by specs 095, 047
...
tree that specs 095, 046, 048 and 051 exist to govern.
...
- **Spec 021's attestation** signs a reproducible corpus seal built from the
...
046 has not already, and claims it in whichever spec creates it.
...
A path under `layout.state_dir` is never witnessed, by construction (spec 036
...
```
L-008  spec '093-the-harness-this-repository-runs' claims 'AGENTS.md', which is
...
diagnosed: an unresolved unit is `W-001` or `W-002` (spec 023), and telling a
...
line, beside the unresolved-unit counts spec 044 added:
...
only, which is exactly the drift it was written to refuse. `VERDICT_SCHEMA_VERSION` does not move: spec 044 §3.6 settled
...
It would also cost the property spec 022 was written for. A claimed file shared
...
- **Sixty-nine declared deliberate**, as `crates/**/*.rs`. They are not
  ...
  index stale for an edit to one, and spec 021's attestation covers a ledger
...
never contributed to any content hash, and spec 021 has been signing an
...
zero-match entries years earlier; spec 047 §4 already defers that idea, and this
is a second reason to want it.

**Folding claimed files into the hash.** §3.4.

**Extending the seal.** Spec 021's attestation covers what the ledger hashes.
Making it cover more is a change to the seal's input set and belongs with the
folding decision, not ahead of it.

**Unresolved units.** `W-001` and `W-002` (specs 023, 050) diagnose a claim that
...
# had. Asserted through `config show` (spec 047), a governed read.
