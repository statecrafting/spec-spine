---
id: "024-resolution-discovery-fixes"
...
depends_on:
  ...
  - "020-keypath-section-anchors" # the eligible-file predicate and §3.2 region promise
amends:
  - "004-codebase-index"          # Makefile `## tag:` determinism; nested-workspace glob base
  - "020-keypath-section-anchors" # fulfill §3.2's foreign-YAML region promise; retire §4's D4 deferral
extends:
...
summary: >
  ...
  region parser, which is exactly the whole-file / region ownership spec 020 §3.2
...
mechanical companion to spec 023 (the unresolved-unit severity policy); the two
...
- **D1 (foreign-YAML regions).** `enumerate_sections` routes any non-workflow
  ...
  020 §3.2, which already states foreign structured configs "keep whole-file /
...
rewrite their bodies): spec 020's foreign-config ownership (D1 fulfils §3.2 and
...
corrections within the existing grammar (the 0.6.0 schema minor is spec 023's).
...
workflow (spec 020's `is_workflow_path` predicate is false) to `region_sections`
...
- **The severity policy** (lifecycle / edge-type warning tiers): spec 023.
