...
references:
  ...
  - { unit: { kind: file, path: "specs/008-coupling-floor-claim-precedence/spec.md" }, role: context }
...
handling spec 033 established for `specs_dir`. Prefix matching is
...
same defect spec 033 fixed in `couple.rs`, where two functions spelled the corpus
...
claimed-path-overrides-bypass precedence (spec 008) into a directory whose entire
...
authority-laundering failure mode spec 023 was careful to close: a unit is never
...
- **D-6 (2026-09-06): `L-006` covers all six ownership-bearing edges, not five.**
  ...
  `supersedes` item carries the unit whose authority transfers (spec 018), so it
