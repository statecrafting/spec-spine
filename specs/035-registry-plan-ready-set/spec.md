---
id: "035-registry-plan-ready-set"
...
references:
  ...
  - { unit: { kind: file, path: "specs/030-dependency-cycle-refusal/spec.md" }, role: context }
  - { unit: { kind: file, path: "docs/design/02-agentic-builder-substrate.md" }, role: context }
summary: >
  `depends_on` records the order specs must be built in, spec 030 guarantees
...
| acyclicity | spec 030 refuses a cycle at compile | the graph can be walked |
...
The walk assumes acyclicity and is entitled to: spec 030 refuses a cycle at
...
**Everything spec 030 §4 excluded, which is why this `amends` it rather than
...
- **D-1 (2026-09-06): `plan`'s cycle guard agrees with `compile`'s by
  ...
  entitled to assume acyclicity because spec 030 refuses a cycle at compile
