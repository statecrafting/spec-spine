...
extends:
  ...
  # the same additive shape specs 011 and 013 used).
  ...
  # contract file (001's surface, as for specs 010/012/031).
...
it (the workflow spec 023's `W-001` tier exists for). Its burn-down lists the
...
1. **Specific.** Some spec's *resolved, ownership-bearing* unit covers the
   ...
   (spec 016) whose subtree contains it; **or** the file names its spec in a
...
being a projection of the shards, spec 010, and every `couple` run pays a full
...
shard, eroding spec 022's conflict-free property for the most common kind of
...
1. **The bypass set still exempts** (floor plus adopter additions), and an
   explicit claim still overrides it (spec 008); such a path has a specific
