---
id: "083-an-amendment-carries-the-acceptance-it-replaces"
title: "An amendment carries the acceptance it replaces"
status: approved
kind: "core"
created: "2026-09-16"
summary: >
  Spec 080 amended spec 079 so that an unresolved claim exits 1 rather than 2,
  and migrated the four assertions that live in Rust. Spec 079's own
...
verify: 079-a-blocking-claim-is-not-a-stale-shard: FAILED at command 9 (exit 1)
```
...
The verb answers 1, and answering 1 is what spec 080 requires:
...
### 1.2 Why spec 080 did not fix this
...
### 3.1 What spec 079's acceptance now is
...
Replacing the block in full rather than the five lines follows spec 082 §3.5: a
...
The `AC-3: stale only` arm MUST keep `test $? -eq 2`. Spec 080 moved one
...
- **A validation that an amendment must declare `amends_verification`.** Not
  ...
  082 §4 declined the neighbouring lint for the same reason.
...
spec touches `settings.json`, which specs 094, 099 and 104 show is a file this
...
(`verification-block-must-fail-before-the-build`, spec 082 §1.1, this one), and
...
> `amends_verification`, so `spec-spine verify 083` builds its plan from that
...
state instead. This block is spec 079's acceptance (§3.1) as well as this
...
```verify:cli
# --- spec 079's acceptance, which this block now holds (3.1) ---
...
grep -qF 'index check >/dev/null 2>&1; test $? -eq 2' specs/079-a-blocking-claim-is-not-a-stale-shard/spec.md
