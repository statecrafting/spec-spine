---
id: "082-an-amended-acceptance-is-the-one-that-runs"
...
extends:
  ...
  - { spec: "043-verify-declared-acceptance", unit: "crates/spec-spine-types/src/verify.rs", nature: additive }
  # 3.1: the version pin moves with the constant it pins.
  - { spec: "043-verify-declared-acceptance", unit: "crates/spec-spine-types/tests/dtos.rs", nature: additive }
...
...
[verify] exit 1
verify: 074-a-governed-read-names-its-version: FAILED at command 13 (exit 1)
```
...
The route this spec builds is the one spec 037 §3.3 already argues for in the
...
`specVersion` and leaves every `shardHash` alone, the shape spec 026 already
established.
...
```
verify: 074-a-governed-read-names-its-version
  acceptance amended by 082-an-amended-acceptance-is-the-one-that-runs (spec 037)
```

A block that runs under another spec's name with no line saying so is the
laundering shape spec 037 §3.2 exists to refuse: the whole value of amending
...
- **`amends_sections`.** The key exists in the grammar and no spec uses it;
  spec 077 D-2 declined to establish the convention. This spec does not revive
...
projection said nothing, which is precisely the discovery path spec 037 §3.3
makes authoritative.
...
block that is 093's acceptance cannot run `verify 093`: spec 043 §3.7's `R-001`
