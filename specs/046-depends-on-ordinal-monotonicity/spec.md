---
id: "046-depends-on-ordinal-monotonicity"
...
extends:
  ...
  # 003's spec.md is not edited (spec 037).
...
summary: >
  ...
  ordinal than the spec declaring it. Spec 030 already ships the harder half of
...
`depends_on` is the scheduling edge. `registry plan` (spec 035) reads it to
partition the corpus into what can be worked on now and what is blocked, and
spec 030 refuses a cycle in it at compile time, because a cyclic scheduling
graph has no answer and a walk over it does not terminate.

What the tool has never checked is direction. Nothing stops
`011-index-hash-slices` from declaring `depends_on: ["041-in-progress-is-in-flight"]`,
...
Ordinal monotonicity is also strictly stronger than the property spec 030
...
additive surface. Spec 030 owns the cycle refusal and is untouched: this spec
...
```
L-007  spec '011-index-hash-slices' depends_on '041-in-progress-is-in-flight',
...
meant to judge, which is the error spec 093 found in the kit's hooks.
...
**Replacing spec 030.** The cycle refusal stays unconditional and unchanged. It
...
> 046` runs 154's block: 146's with one skills line restated for spec 150.
