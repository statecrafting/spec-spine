---
id: "120-the-engine-ships-governance-not-an-environment"
...
extends:
  ...
  - { spec: "006-init-scaffold", unit: "crates/spec-spine-core/src/scaffold.rs", nature: reductive }
  ...
  - { spec: "039-declared-state-dir", unit: "crates/spec-spine-types/src/config.rs", nature: additive }
  ...
  - { spec: "112-nothing-reruns-a-merged-acceptance", unit: "scripts/verify-sweep.sh", nature: additive }
  ...
  - { spec: "047-harness-rules-name-the-legitimate-edits", unit: ".claude/rules/orchestrator-rules.md", nature: additive }
...
`spec-spine init` MUST fail as an unknown subcommand, which spec 063 maps to
exit 3.
...
| File | Source |
|---|---|
| `spec-spine.toml` | the documented starter config, config-aware (spec 061) |
...
047 and 065 are environment, and the environment has an owner.
...
spec 064's composite chain, spec 089's explicit `if`/`then`/`else` guards,
spec 114's `OWNERSHIP` and `COUPLE` controls with their refusal of an
unrecognized value and their announced skips, and spec 072's base-ref
...
cancelled by it, for the reason spec 039 3.5 gives about the state root.
...
the precedent spec 074 3.6 set and spec 048 D-7 recorded, for exactly this
...
2. **The record is in this spec.** A predecessor is not edited to mention its
   successor (spec 040 3.1). Its frontmatter loses a claim on a path that is
   gone; why is here.
...
pass, and `scripts/verify-sweep.sh` (spec 112) runs the whole corpus's declared
...
- **Replacement.** This spec declares the predecessor in `amends_verification`
  ...
  (spec 103 3.2). Used where the block's subject is the removed surface.
...
Spec 064's argument is that the chain has one definition and that CI runs the
artifact rather than a restatement of it; spec 114 D-1 records what happened
...
a new parent is not a reason to stop committing it. `state_dir` is spec 039's
...
! git check-ignore -q .statecraft/derived/spec-registry/by-spec/120-the-engine-ships-governance-not-an-environment.json
...
python3 -c "import json; d=json.load(open('${TMPDIR:-/tmp}/ss120-show.json')); assert d['id'] == '120-the-engine-ships-governance-not-an-environment', d"
