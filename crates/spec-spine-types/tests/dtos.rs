...
fn schema_versions_are_pinned() {
    ...
    // `sectionDigests` map on every record; 1.5.0: additive MINOR (spec 109),
    // optional `impacts` and `conflicts`, each `obligation` normalized to its
    // full qualified form; 1.6.0: additive MINOR (spec 110), optional
    // `interfaceReferences`, a spec's cross-corpus citations pinned to
    // another repository's content hash; 1.7.0: reserved for spec 114, built
    // concurrently on another branch; 1.8.0: additive MINOR (spec 111),
    // optional `moves`, a spec's declared relocation, split, merge or
    // removal of a path. A consumer that knows 1.x keeps working, which is
    // the MINOR rule.
