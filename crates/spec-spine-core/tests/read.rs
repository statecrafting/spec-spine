...
fn a_stamped_document_carries_the_read_axis_version() {
    ...
    // The axis started at 0.1.0 (§3.4) and has moved three times, additively:
    // spec 102 added `status` to a `plan` ready entry, spec 106 added the
    // `obligation` document, and spec 107 the resolved closure. Pinned, so the
    // next move is a decision someone writes down rather than a constant that
    // drifts.
    assert_eq!(
        READ_SCHEMA_VERSION, "0.4.0",
