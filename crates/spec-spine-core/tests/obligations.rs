...
fn a_spec_without_the_key_carries_no_obligations_and_its_shard_hash_is_the_file_hash() {
    ...
    // Spec 109's extends edge on this file: an additive registry MINOR after
    // 106's shipped (109's own `impacts`/`conflicts`) moves this pin the same
    // way 106 itself once moved a pin in 082's territory (106 D-8).
    assert_eq!(out.shards.spec_shards[0].spec_version, "1.5.0");
}
