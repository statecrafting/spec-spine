...
1. **Parallel reads.** Dispatch the following simultaneously (nothing here
   ...
   - `spec-spine index coverage`: which source files no spec specifically claims (spec 029; non-fatal, exit 1 if the index is stale)
...
- **`1` (a finding):** the report lines say which. A `STALE` line: *check the `--version` read from step 1 before believing it* (see **Stale binary** below), then say **which tree** the output named, report "Spec registry: stale, run `spec-spine compile` and commit" or "Codebase index: stale, run `spec-spine index`" accordingly, **and name the drifted shards from its stderr**, then continue; the lifecycle counts come from the committed ledger and are therefore the stale ones, so say so. An `INVALID` line: the corpus fails validation; surface the violations and report the lifecycle counts as **unverified** (they come from a ledger nobody can say corresponds to the corpus), and fixing the violations is the first task of the session. INVALID outranks STALE in the same report, because staleness is not meaningful against a corpus that does not validate. An `UNRESOLVED CLAIM` line (or, with `--fail-on-unresolved`, a refused unresolved-unit diagnostic): a spec claims a unit that does not resolve, which regenerating does not clear.
- **`2` (refused):** the configuration is invalid or `[meta] required_version` is not satisfied by this binary, and nothing was read. Treat freshness as unknown for **both** trees and report stderr verbatim. (A binary before 0.26.0 spent 2 on staleness; its report still says `STALE`.)
- **`3` (usage) or `4` (failed):** a read that could not be performed has not answered. Treat freshness as unknown for **both** trees, report stderr verbatim, and continue. Never report "fresh" for an exit code you did not recognize. (A binary before 0.26.0 spent 3 on I/O, parse, schema and config failures.)
