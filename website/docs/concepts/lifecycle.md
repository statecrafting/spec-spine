...
035, 041, 043 and 044 they are **lifecycle keys with mechanical consequences**,
...
| `depends_on` | spec ids | Which specs must land first. A cycle is a compile-time refusal (spec 030); the acyclic graph orders `registry plan`. |

## Done is not self-authored

The central rule (spec 038): **a spec that declares itself complete is held to
...
that does not exist yet (spec 023), so a spec can be filed, and even ratified,
...
| approved | **in-progress** | **yes** (spec 041) | `W-001` warning |
...
- `approved` + `in-progress` is in flight (spec 041). A corpus that ratifies
...
`spec-spine registry plan` (spec 035) partitions the registry into what a
...
Spec 037 fixes the authoring rule for `amends`: the edge is declared in the
...
The constitution is the case `amends` does not cover (spec 040). It is not a
...
Spec 040 gives constitution principle V its operational half. Code adopted from
