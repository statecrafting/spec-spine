...
| Derived | `.statecraft/derived/` | Generated by `spec-spine compile`; never edit by hand |

The behavioral rules in `AGENTS.md`'s `## Rules` section apply: execute steps in order, stop at checkpoints, keep the working tree green, halt on failure, refresh derived artifacts before opening a PR.
